...
from wrapper import MaxAndSkipWrapper, WarpFrameWrapper, ScaledFloatFrameWrapper, FrameStackWrapper, FinalEvalRewardEnv,SparseRewardWrapper,CoinRewardWrapper
...
# mario环境
def wrapped_mario_env(version=0, action=7, obs=4):
...
def main(cfg, args, seed=0, max_env_step=int(7e8)):
    # Easydict类实例，包含一些配置
    cfg = compile_config(
        cfg,
    ...
    # 1. 构建日志路径：直接使用 os.path.join
    log_path = os.path.join(cfg.exp_name, 'log', 'serial')
    
    # 2. 检查并创建目录 (防止 tensorboard 报错)
...
if __name__ == "__main__": # python mario_dqn_main.py -s 0 -v 0 -a 2 -o 4
